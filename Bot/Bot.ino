...
GeoLoc gpsdump(TinyGPS &gps) {
  ...
  Serial.print(BotLoc.lat, 7); Serial.print(", "); Serial.println(BotLoc.lon, 7);
...
void driveTo(struct GeoLoc &loc, int timeout) {
  nss.listen();
  GeoLoc BotLoc = checkGPS();
  bluetoothSerial.listen();

  if (BotLoc.lat != 0 && BotLoc.lon != 0 && enabled) {
    float d = 0;
    ...
    do {
      ...
      d = geoDistance(BotLoc, loc);
      float t = geoBearing(BotLoc, loc) - geoHeading();
      
      Serial.print("Distance: ");
      Serial.println(geoDistance(BotLoc, loc));
    
      Serial.print("Bearing: ");
      Serial.println(geoBearing(BotLoc, loc));
